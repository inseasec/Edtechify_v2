...
export default function CareersSelectedPanel() {
  const dispatch = useDispatch()
  const navigate = useNavigate()
  const base = useAdminBasePath()
  const { title, description } = CAREER_SECTIONS.selected
  ...
  const sortedApplicants = useMemo(() => {
    return [...allApplicants].sort((a, b) => {
      const ta = new Date(a.createdAt ?? a.updatedAt ?? a.applicationDate ?? 0).getTime()
      const tb = new Date(b.createdAt ?? b.updatedAt ?? b.applicationDate ?? 0).getTime()
      return tb - ta
    })
  }, [allApplicants])
  ...
  const {
    ...
    nameSortOrder,
    setNameSortOrder,
  } = useApplicantFilters(sortedApplicants, ITEMS_PER_PAGE, PIPELINE_STAGE.selected)

  const { calculateAge, normalizeSalary, formatSalary } = helpers

  useEffect(() => {
    dispatch(fetchApplicantsByStatus(PIPELINE_STAGE.selected))
  }, [dispatch])
  ...
  const openApplicant = (row) => {
    navigate(`${base}/careers/applicant/${row.id}`, { state: { applicant: row, from: 'SELECTED' } })
  }
    ...
    <div className="w-full space-y-4 text-left">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button
          type="button"
          onClick={() =>
            navigate(`${base}/careers/videos`, { state: { status: PIPELINE_STAGE.selected } })
          }
          title="Video view"
        ...
        <>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div className="border-b border-slate-100 px-3 py-2">
              <span className="text-sm font-medium text-slate-700">Selected candidates</span>
            </div>
                  ...
                  {paginatedApplicants.length === 0 && (
                    <tr>
                      <td colSpan={9} className="px-4 py-8 text-center text-slate-500">
                        No selected applicants match these filters, or the list is empty.
                      </td>
                    </tr>
                  )}
                  {paginatedApplicants.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-2 text-slate-600">
                        {formatSalary(normalizeSalary(row.currentSalary))}
                      </td>
                      <td className="px-2 py-2 text-slate-600">
                        {formatSalary(normalizeSalary(row.expectedSalary))}
                      </td>
                    </tr>
                  ))}
                </tbody>
